...
class StepOne extends BasicForm
{
    ...
    protected $rules = [
        ...
        'info.border_no' => 'required',
        'info.passport_no' => 'required',
        'info.end_id_number' => 'required',
        'info.end_saudi_authority' => 'nullable',
        'info.end_medical_insurance'=>'nullable',
        ...
        'info.bank_iban'=> 'required',
        'info.bank_account'=>'required',
        ...
        'relative.full_name' => 'required',
        'relative.phone' => 'required',
    ...
    public function mount($employee_id = null) {
        if($employee_id != null) {
            ...
            $this->cities = State::where('country_id',$this->employee->country_id)->pluck('name','id')->toArray();

            if($this->employee->motherAddress) {
                $city =  State::where('id', $this->employee->motherAddress->city_id)->first();
                
               if( $city?->country_id == null){
                    $this->cities_2 = State::pluck('name','id')->toArray();
                }else{
                $this->cities_2 = State::where('country_id',$city?->country_id)->pluck('name','id')->toArray();
                }
...
//        dd("للاثممخ");
        ...
        if($this->id_number_photo) {
            $this->info['id_number_photo'] = uploadFile($this->id_number_photo, "employees", $emp->id,"id_number_photo");
        }
...
//dd($this->personalImage);
        ...
        if($this->id_number_photo) {
            $this->info['id_number_photo'] = uploadFile($this->id_number_photo, "employees", $this->employee_id,"id_number_photo");
        }
    ...
    public function updatedidNumberPhoto() {
        $this->validate([
            'id_number_photo' => 'image|max:1024|mimes:png,jpg,jpeg', // 1MB Max
        ]);

        $this->info['id_number_photo'] = $this->id_number_photo;
    }
    ...
    public function updatedmotherAddressCountryId() {
        $this->cities_2 = State::where('country_id', $this->motherAddress['country_id'])->pluck('name','id')->toArray();
    }
}