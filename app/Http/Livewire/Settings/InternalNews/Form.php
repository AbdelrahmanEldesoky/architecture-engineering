    ...
    public function save() {
        ...
        if(! array_key_exists('id', $this->internal)) {
            $users = User::whereHas('employee',function ($query){
                $query->whereHas('workAt',function ($q) {
                    $q->where('workable_type', 'departments')->where('workable_id', $this->internal['department_id']);
                });
            })->get();//all users
            $data = [];
          ...
          $fcm->sendNotification($users->modelKeys(),"تم اضافة تعاميم جديدة", $this->internal['title']['ar'], null, null, null, "users");
