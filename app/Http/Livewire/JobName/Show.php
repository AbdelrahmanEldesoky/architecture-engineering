...
class Show extends BasicTable
{
    ...
    {
        $jobNames = EmploymentData::with(['jobType','jobName','employee','employee.attachments'])
                ->where('job_name_id',$this->jobTypeId)
                ...
                ->when($this->jobNameID,function ($query){
                    $query->where('job_name_id',   $this->jobNameID);
                })
    ...
    {
        $job_name_title = JobName::findOrFail($this->jobTypeId);
        $jobNames = $this->search();
        $jobNamesSelect = JobName::get();
        $JobTypes = JobType::get();
        $JobPage = $this->JobtypePage;
        return view('livewire.job-name.show',compact('jobNames','jobNamesSelect','JobTypes','JobPage','job_name_title'));
    }
