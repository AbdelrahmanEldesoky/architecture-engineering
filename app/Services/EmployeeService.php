...
class EmployeeService extends MainService {
    ...
    public function store($data) {
          ...
          $data['name'] = $data['first_name'] . ' ' . $data['second_name'] . ' ' .$data['last_name'];
    ...
    public function storeDues($employee_id, $name, $values) {
        ...
        if($empDue) {
            ...
            if ($name === 'medical-insurance' ||$name === 'airplane' || $name === 'transfer-sponsership') {
                if(is_array($values)){
    ...
    public function storeRelative($data, $id)
    {
        ...
        if ($relative) {
            ...
            Relative::create([
                ...
                'type' => isset($data['relative_Type']) ? $data['relative_Type'] : null,
    ...
    public function search($term = null, $draft = 0, $branch = null) {
        if (!empty($term)) {
             return Employee::where('draft', $draft)
                ...
                ->where(DB::raw("CONCAT(first_name, ' ',second_name,' ',last_name)"), 'like', '%' . $term . '%')
