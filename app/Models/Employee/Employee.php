...
class Employee extends MainModelSoft
{
    ...
    protected $fillable = ['email', 'phone', 'first_name', 'last_name', 'name', 'second_name', 'country_id', 'city_id', 'user_id', 'address', "shift_id", 'has_overtime'
    ,'branch_id', 'management_id', 'department_id', 'last_seen', 'latitude', 'longitude'];
    ...
    {
        return $this->hasOne(EmployeeTrack::class);
    }
    ...
    public function getTodayAttendanceByDay($day)
    {
        // $dayCreatedAt = Carbon::parse($day->created_at)->fomrat('Y-m-d');
        // $shiftStart = $dayCreatedAt .' '. Carbon::parse($day->start_in)->format('h:i:s');
        // $shiftHours = round(abs((strtotime($day->end_in) - strtotime($day->start_in)) / 3600 ), 2);
        // $shiftEnd = Date('Y-m-d h:i:s', strtotime($shiftStart , strtotime("+ " . $shiftHours . " hours")));

        // return $shiftEnd;

        // egypt timezone => bade2 el shift mn 1 bleel youm 10-10-2023 w hay5ls 8 el sob7 youm 10-10-2023
        // utc   timezone => bade2 el shift mn 10 bleel youm 9-10-2023 w hay5ls 5 el sob7 youm 10-10-2023

        // low dlw2ty abl el sa3a 3 AM haygeebo ba3d keda msh haygebo .. yeb2a el mafrod el attendance hayege day between [];
        // typ low hwa b2a haybd2 tany el sa3a 9 el sob7 keda hayb2a leh 2 attendance .. bas l2a l2no hayt3ml create b tare5 tany
