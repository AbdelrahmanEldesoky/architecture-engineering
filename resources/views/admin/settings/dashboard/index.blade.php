    ...
    <div class="section">
        ...
        <div class="collapse " id="dashboard-setting" style="">
            ...
            @if (havePermissionTo('dashboardSetting.cities.view'))
                <a href="{{ route('admin.settings.dashboard.state.index') }}"
                   class="btn btn-primary light w-100 arrow-btn mb-2">{{ __('names.state-setting') }}</a>
            @endif
            @if (havePermissionTo('dashboardSetting.cities.view'))
            ...
            @endif
            @if (havePermissionTo('dashboardSetting.cities.view'))
            <a href="{{ route('admin.settings.dashboard.date-end.index') }}"
               class="btn btn-primary light w-100 arrow-btn mb-2">{{ __('تنبيهات تواريخ الانتهاء') }}</a>
        @endif
        </div>
    </div>
</x-admin-app>
