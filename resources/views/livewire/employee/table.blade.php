<div>
    ...
    <div class="my-2 d-flex gap-2">
        ...
        <button wire:click="changeDraft()" type="button" class="btn btn-outline-primary btn-sm position-relative">
            @if ($draft == 0)
                <i class="bx bx-box bx-sm"></i> {{ __('مسودة') }}
        ...
        <div class="row">
            <div class="col-md-12 ">
                <div class="w-100 pt-3 carousal_container">
                   ...
                   <div class="d-flex ">
                    <div class="wrapper  justify-content-between w-100">
                        <nav class="nav-tabs" >
                                
                            <a class=" {{ $branchId == 0 ? 'active' : '' }}" aria-current="page"
                            href="{{ route('admin.employees.index') }}">
                            {{ __('names.all') }}
                        </a>

                        @foreach ($branches as $key => $branch)
                            <a class=" {{ $branchId == $key ? 'active' : '' }}" aria-current="page"
                    ...
                    <div >
                        <button class="btn btn-outline-primary mx-2 mb-2 p-0 " style="width:150px" type="button"
                            data-bs-toggle="collapse" data-bs-target="#filter" aria-expanded="false" aria-controls="filter">
                            <i class='bx bx-filter-alt w-100 py-2 fs-4'>{{ __('names.filter') }} </i>    
                        </button>
                      </div>
                   </div>
                    <div class="row collapse mb-2" id="filter" wire:ignore>
                        <div class="col">
                            <x-input-label :value="__('نوع الزظيفة')"></x-input-label>
                            <select wire:model="jobType" class="form-select">
                        ...
                        <div class="col">
                            <x-input-label :value="__('المسمي الوظيفي')"></x-input-label>
                            <select wire:model="jobName" class="form-select">
                                <option value="">{{ __('names.all') }}</option>
            ...
            <x-table responsive="true">
                ...
                <tbody>
                    @forelse ($employees as $key=>$employee)
                        <tr>
                            <td><img src="{{  url('assets/images/normal.svg') }}" alt=""></td>
                            ...
                            <td>
                                <p class="{{ $employee->draft == 1 ? 'status stopped' : 'status active' }}">
