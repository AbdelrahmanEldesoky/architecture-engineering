    ...
    <div class="row my-3 d-flex">
        ...
        <div class="col d-flex flex-row-reverse ">
            ...
            @if (havePermissionTo('dashboardSetting.cities.create'))
                <a href="{{ route('admin.settings.dashboard.state.create') }}">
                    <x-button class="  d-flex justify-content-center align-items-center">
                        <i class='bx bx-plus-circle bx-sm'></i>
                        {{ __('message.add', ['model' => __('محاظفة')]) }}
                    </x-button>
                </a>
            @endif
        </div>
    ...
    <div class="row">
        <x-table :responsive="true">
            ...
            @forelse($states as $key => $state)
                <tr>
                    <td><a href="{{ route('admin.settings.dashboard.state.edit', $state->id) }}">
                            {{ Str::limit($state->name, 50) }}
                        </a>
                    </td>
                    <td>
                        {{ $state->country->name }}
                    ...
                    <td>
                        <div class=" limit-2">
                            @if (havePermissionTo('dashboardSetting.cities.edit'))
                                <a href="{{ route('admin.settings.dashboard.state.edit', $state->id) }}"
                                   class="px-1">
                                    <i class='bx bxs-edit bx-sm text-gray'></i>
                                </a>
                            @endif
                            @if (havePermissionTo('dashboardSetting.cities.delete'))
                                <a href="#" class="px-1" wire:click.prevent="delete({{ $state->id }})">
